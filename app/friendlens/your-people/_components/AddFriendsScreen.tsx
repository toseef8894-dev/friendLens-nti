...
export default function AddFriendsScreen({ single = false }: AddFriendsScreenProps) {
  ...
  const [modalQueue, setModalQueue] = useState<Friend[]>([])
  ...
  function advanceQueue(queue: Friend[]) {
  ...
  const activeFriend = modalQueue[0] ?? null
    ...
    <>
      ...
      {activeFriend && (
        <EditFriendModal
          open={true}
          friend={activeFriend}
          stackCount={modalQueue.length}
          onClose={() => advanceQueue(modalQueue)}
          onSaved={() => advanceQueue(modalQueue)}
          onDeleted={() => advanceQueue(modalQueue)}
        />
      )}
    </>
  )
